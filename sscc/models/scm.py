...
class SCM(nn.Module):
    ...
    def loss_function(self, outs, batch, loss_type, lambda_unsup, threshold, **kwargs):
        ...
        if len(batch) == 3:
            ...
            # 2) unsupervised loss
            # either: 1.) naive approach with pseudo labels
            if loss_type == 'ce_loss':
                ...
                #TODO: check if this eats up a lot of runtime and do only every XYZ steps in case it does
                # cluster assignment for quality of ALL weak pseudo labels
                k = out_weak.shape[1]
                ...
                if int(sum(max_prob > threshold)) > 1:
                    # pl_conf_acc = sum(batch['cm_train']['y'][max_prob > threshold] == max_class[max_prob > threshold]) / sum(max_prob > threshold)
                    ...
                    # cluster assignment for quality of regularization quality weak pseudo labels
                ...
                # # or: 2.) less naive approach with pseudo constraints
                # train_target_pseudo = self.pseudo_constraints(out_weak, threshold=threshold)
                # prob1_pseudo, prob2_pseudo = PairEnum(out_strong)
                # loss_unsup = self.criterion(prob1_pseudo, prob2_pseudo, simi=train_target_pseudo)
