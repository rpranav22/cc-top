...
class Evaluator(object):
    ...
    def plot_confmat(self, title: str='', true_k: int=10):
        ...
        fig.suptitle(title)
        fig.set_size_inches(1 + int(5 * self.k / 10), int(5 * true_k / 10))
        sn.set(font_scale=1.0)
        heat = sn.heatmap(df_cm, annot=True, annot_kws={"size": 10}, fmt='d', cmap="YlGnBu", ax=axs)
        axs.set_xlabel('prediction')
        axs.set_ylabel('true cluster assignment')
        plt.tight_layout()
        return fig